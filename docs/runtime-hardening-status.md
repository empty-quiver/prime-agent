...
- Detached child runs now retain a durable parent receipt beyond spawn admission. A crashed parent recovers the child as outcome unknown and blocks new work until host reconciliation; successful terminal receipts restore child-wait completion without respawning. Standalone child-runtime crashes and cleanup uncertainty remain conservative unknown outcomes.
- Failure notices carry typed timeout, cancellation, provider failure, worker crash or startup failure classifications in their existing custom-message payload; uncertainty never authorizes repeating the child task. No daemon command/event envelope or client capability changed.
...
- Completed receipt files are retained on disk with a fail-closed 100,000-record capacity and 1,024 pending-operation cap. Directory enumeration streams entries; bounded regular-file reads reject oversized records and symlinks before parsing. Inbox reads use the same bounded reader. No recovery or deduplication evidence is silently pruned. This journal does not provide exactly-once semantics for remote services or contain an escaped process.
- `recoveryError` exposes receipt persistence faults to supervisor health, including failures during detached child settlement.
